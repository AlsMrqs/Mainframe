...
displayGrid :: Magisterium.Game -> GLUT.DisplayCallback
displayGrid game = renderLines (newRGB 0.3 0.3 0.3) Magisterium.grid

displayTrail :: Magisterium.Game -> GLUT.DisplayCallback
displayTrail game = do
    case (projectile . offensive . currentRound) game of
        Nothing    -> return ()
...
displayAim game = do
    GLUT.Size dltx dlty <- GLUT.get GLUT.windowSize
    t <- Math.getTime
    let lock (x,y) = map (Math.sumPoint (x,y,0)) (Magisterium.squaredAim (dltx,dlty))
...
displayOffensive game = do
    ...
    case (projectile . offensive . currentRound) game of
        Nothing     -> return ()
        Just (t0,f) -> do
            let t' = if Magisterium.isWaiting game 
                    then Magisterium.time game else t
...
displayTrack game = do
    ...
    case (projectile . offensive . currentRound) game of
        Nothing     -> return ()
        Just (t0,f) -> 
            case (analysis . defensive . currentRound) game of
                Nothing -> return ()
                Just f' -> do
                    let t' = if Magisterium.isWaiting game 
                            then Magisterium.time game else t
